...
def identify_atoms_in_ml_region(positions, 
    ...
    # formic_acid_positions = positions[formic_acid_indices]
    formic_acid_positions = positions[formic_acid_carbon_indices]
    ...
    formic_acid_positions = formic_acid_positions * 10
    ...
    pairs, pair_distances = capped_distance(formic_acid_positions, 
                                            water_positions, 
    ...
    pairs[:, 1] = pairs[:, 1] + len(formic_acid_indices)
    ...
    water_molecule_indices = np.array_split(surrounding_water_indices, len(surrounding_water_indices)/3)
...
# Create an OpenFF Molecule object for formic acid from SMILES
from openff.toolkit.topology import Molecule
molecule = Molecule.from_smiles('OC=O')

# Create the SMIRNOFF template generator
from openmmforcefields.generators import SMIRNOFFTemplateGenerator
gaff = SMIRNOFFTemplateGenerator(molecules=molecule)
...
####################################
# IF STARTING FROM PREVIOUS STRUCTURE
# # read in the initial formic acid and water box structure
formic_acid = PDBFile('./mm_fa_waters_npt_run_final.pdb')
modeller = Modeller(formic_acid.topology, formic_acid.positions)

####################################
# IF NOT STARTING FROM PREVIOUS STRUCTURE

# formic_acid = PDBFile('./formic_acid_rdkit.pdb')

# # initial density is not correct (is 0.88 g/cm^3). 
# # thus, we redfine the box vectors to get a density of 1.0 g/cm^3
# # box vectors are in nm
# mass_water = 18.01528 # g/mol
# num_water  = 1000
# mass_formic_acid = 46.0254 # g/mol
# num_formic_acid  = 1
# AVOGADRO_CONSTANT_NA = 6.022140857e+23
# target_density = 1.0 # g/cm^3

# total_mass = (num_water/AVOGADRO_CONSTANT_NA)*mass_water + (num_formic_acid/AVOGADRO_CONSTANT_NA)*mass_formic_acid # g  
# volume = total_mass/target_density # cm^3
# volume = volume*1e21 # nm^3
# box_vector = volume**(1/3) # nm

# modeller = Modeller(formic_acid.topology, formic_acid.positions)
# modeller.addSolvent(forcefield,
#        boxVectors=(Vec3(box_vector, 0.0, 0.0), Vec3(0, box_vector, 0), Vec3(0, 0, box_vector)),
#        model='tip3p')
####################################

# now, we will identify the atoms that are within the ML region 
# first, we get the indices for the formic acid atoms and the water atoms

total_num_atoms = modeller.topology.getNumAtoms()
num_formic_acid_atoms = 5
formic_acid_indices = np.array(list(range(num_formic_acid_atoms)))
waterAtomIndices = np.array(list(range(num_formic_acid_atoms, total_num_atoms)))
assert len(formic_acid_indices) + len(waterAtomIndices) == total_num_atoms

formic_acid_carbon_indices = []
for atom in modeller.topology.atoms():
    if atom.element._name == 'carbon':
        formic_acid_carbon_indices.append(atom.index)
        break
formic_acid_carbon_indices = np.array(formic_acid_carbon_indices)
...
# atoms_within_cutoff does not include the formic acid atoms (excluding carbon)
# include the non-carbon formic acid atoms in the ml region
atoms_within_cutoff = np.array(atoms_within_cutoff.tolist() + np.setdiff1d(formic_acid_indices, formic_acid_carbon_indices).tolist())
atoms_within_cutoff = np.sort(atoms_within_cutoff)
print('Atoms within the cutoff (excluding formic acid carbon):', atoms_within_cutoff)

ml_region_particles = atoms_within_cutoff.tolist() + formic_acid_carbon_indices.tolist()
...
for atom in modeller.topology.atoms():
    ...
    if atom.index in formic_acid_indices:
        atom_type = atom.element.symbol
        atom_name = atom.name

        # add the Lennard-Jones and charge parameters for the particles
        # formic acid values are from GAFF
        if atom_name == 'O1':
            sigma = 0.2997159987248637 * nanometer
            epsilon = 0.8764372596155737 * kilojoules_per_mole
            charge = -0.6151 
        elif atom_name == 'C1':
            sigma = 0.34806468869450646 * nanometer
            epsilon = 0.3635030558377792 * kilojoules_per_mole
            charge = 0.6319 
        elif atom_name == 'O2':
            sigma = 0.3039812205065809 * nanometer
            epsilon = 0.8795023257036865 * kilojoules_per_mole
            charge = -0.546 
        elif atom_name == 'H1':
            sigma = 0.053453923088366904 * nanometer
            epsilon = 5.157198260534728e-05 * kilojoules_per_mole
            charge = 0.443
        elif atom_name == 'H2':
            sigma = 0.2441922754112159 * nanometer
            epsilon = 0.06191251006677376 * kilojoules_per_mole
            charge = 0.0862
        else:
            raise ValueError('Atom type not recognized!')
        nonbonded_force.addParticle(charge, sigma, epsilon)
    
    # parameter values are from TIP3P
    elif atom.index in waterAtomIndices:
...
PDBFile.writeFile(
    ...
    open('prod_mlmm_fa_waters_initial_frame.pdb', 'w'))
...
np.savetxt('ml_region_atoms_mlmm_simulation_fa_waters_run01.txt', atoms_to_save, fmt='%d')
...
# the spherical cavity radius should be such that the density of the formic acid plus waters
# is close to 1.0 g/cm^3
num_waters = (atoms_within_cutoff.shape[0] - 4) // 3
num_formic_acid = 1
...
central_formic_acid_atoms: GROUP ATOMS=1-5
central_formic_acid_carbon_atom: GROUP ATOMS={','.join(map(str, carbon_atom_indices+1))}

# define atom groups for the coordination number
# formic acid oxygen index: 
acidic_oxygen: GROUP ATOMS={','.join(map(str, formic_acid_oxygen_atom_indices+1))}

# hydrogen indices
hydrogens: GROUP ...
    ATOMS={','.join(map(str, hydrogen_atom_indices+1))}
...
...
# calculate the distances between the COM of the ml central water and the ml region
d: DISTANCES GROUPA=central_formic_acid_carbon_atom GROUPB=ml_region_atoms 
...
d_outside: DISTANCES GROUPA=central_formic_acid_carbon_atom, GROUPB=water_atoms_outside_cutoff
...
# # moving restraint for umbrella sampling simulations
# restraint: MOVINGRESTRAINT ...
#     ARG=c1    
#     STEP0=0        AT0=0.2  KAPPA0=0.0
#     STEP1=40000     AT1=0.2  KAPPA1=0.0
#     STEP2=44000     AT2=0.2  KAPPA2=1447.275
# ... 
...
PDBFile.writeFile(simulation.topology,
                  simulation.context.getState(getPositions=True, enforcePeriodicBox=True).getPositions(),
                  open('prod_mlmm_fa_waters_0.5nm_shell_run01_initial_frame.pdb', 'w'))

# create output files
outfile = os.path.join('prod_mlmm_fa_waters_0.5nm_shell_run01.txt')
dcdfile = os.path.join('prod_mlmm_fa_waters_0.5nm_shell_run01.dcd')
pdbfile = os.path.join('prod_mlmm_fa_waters_0.5nm_shell_run01.pdb')
checkpointfile = os.path.join('prod_mlmm_fa_waters_0.5nm_shell_run01.chk')
...
PDBFile.writeFile(simulation.topology, positions,
                open('prod_mlmm_0.5nm_shell_final.pdb', 'w')
                )
